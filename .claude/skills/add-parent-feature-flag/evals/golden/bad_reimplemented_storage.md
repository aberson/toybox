<!-- BAD OUTPUT — DEFECT: core module re-implements string/bool storage instead of delegating to FeatureFlagSetting -->
<!-- Violates SKILL.md lines 46,84: "Do NOT re-implement storage — FeatureFlagSetting owns ... storage" -->
...
def get(conn: sqlite3.Connection) -> bool:
    ...
    row = conn.execute(
        "SELECT value FROM settings WHERE key = 'starfield_background_enabled'"
    ).fetchone()
    if row is None:
        return STARFIELD_BACKGROUND_ENABLED_DEFAULT
    return row[0] == "true"
...
def set(conn: sqlite3.Connection, value: bool) -> bool:  # noqa: A001 -- mirrors test surface
    ...
    conn.execute(
        "INSERT INTO settings (key, value) VALUES ('starfield_background_enabled', ?) "
