<!-- BAD OUTPUT — DEFECT: added If-Match-Version + 409 optimistic concurrency to a settings PUT -->
<!-- Violates SKILL.md line 94: "No If-Match-Version / no 409. Settings PUTs are last-write-wins ... Do not add a version token." -->
...
GET unauthenticated; PUT parent-scoped; If-Match/409 optimistic concurrency on PUT; no WS broadcast; 422 detail `invalid_starfield_background_enabled`; `check_same_thread=False` kept.
...
def put_starfield_background_enabled_endpoint(
    ...
    if_match_version: Annotated[int, Header(alias="If-Match-Version")],
) -> StarfieldBackgroundEnabledResponse:
    """Persist ``value`` and return the canonical bool."""
    stored_version = core_starfield_background_enabled.get_version(conn)
    if if_match_version != stored_version:
        raise HTTPException(
            status_code=409,
...
```ts
  async getStarfieldBackgroundEnabled(
    ...
    return this.request<FeatureFlagResponse>(
      "/api/settings/starfield-background-enabled",
