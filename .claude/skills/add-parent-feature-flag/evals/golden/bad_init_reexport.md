<!-- BAD OUTPUT — DEFECT: added a spurious re-export of the flag in core/__init__.py -->
<!-- Violates SKILL.md line 84: "core/__init__.py is docstring-only — do NOT re-export there" -->
...
Imported only `FeatureFlagSetting` from `_feature_flag`. Also edited `src/toybox/core/__init__.py` to re-export: `from .starfield_background_enabled import get, set, STARFIELD_BACKGROUND_ENABLED_DEFAULT`.
