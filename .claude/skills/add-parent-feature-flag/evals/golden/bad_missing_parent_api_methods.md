<!-- BAD OUTPUT — DEFECT: parent/api.ts getStarfieldBackgroundEnabled/setStarfieldBackgroundEnabled pair never added (Step 8a skipped) -->
<!-- Violates SKILL.md lines 291-314: "add a getter + setter method pair on ApiClient" -->
...
(8a) `frontend/src/parent/api.ts` — no `ApiClient` changes needed; the bootstrap fetch and toggle wiring below already reference the flag, so proceed straight to 8b.
