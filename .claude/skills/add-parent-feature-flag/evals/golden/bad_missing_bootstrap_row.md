<!-- BAD OUTPUT — DEFECT: App.tsx fetches the flag but omits the flagBootstrapResults row (value never folded into state) -->
<!-- Violates SKILL.md lines 320-323: "WITHOUT this row the parent UI never reads the persisted value ... This is the silent gap." -->
...
(8b) `frontend/src/parent/App.tsx` — added the bootstrap fetch (destructuring tuple + `Promise.allSettled` array in matching order):
