<!-- BAD OUTPUT — DEFECT: kebab path abbreviated (starfield-bg-enabled) instead of full mechanical kebab -->
<!-- Violates SKILL.md line 40: "Kebab derivation is mechanical and test-enforced ... Do not abbreviate." -->
...
class StarfieldBackgroundEnabledResponse(BaseModel):
    """Wire shape for ``GET`` and ``PUT`` ``/api/settings/starfield-bg-enabled``."""

    value: bool


class StarfieldBackgroundEnabledUpdate(BaseModel):
    """Request body for ``PUT /api/settings/starfield-bg-enabled``."""

    value: bool


@router.get("/starfield-bg-enabled", response_model=StarfieldBackgroundEnabledResponse)
...
@router.put("/starfield-bg-enabled", response_model=StarfieldBackgroundEnabledResponse)
...
> = {
  ...
  starfield_background_enabled: "/api/settings/starfield-bg-enabled",
};
```
...
(8a) `frontend/src/parent/api.ts` — added the `getStarfieldBackgroundEnabled` / `setStarfieldBackgroundEnabled` pair:

```ts
  async getStarfieldBackgroundEnabled(
    opts: RequestOptions = {},
  ): Promise<FeatureFlagResponse> {
    return this.request<FeatureFlagResponse>(
      "/api/settings/starfield-bg-enabled",
      { method: "GET", signal: opts.signal },
    );
  }

  async setStarfieldBackgroundEnabled(
    value: boolean,
    opts: RequestOptions = {},
  ): Promise<FeatureFlagResponse> {
    return this.request<FeatureFlagResponse>(
...
```
```ts
        api.getStarfieldBackgroundEnabled({ signal: aborter.signal }),
```
```ts
        ["starfield_background_enabled", starfieldBackgroundResult],
```
...
```ts
  {
    ...
    setter: "setStarfieldBackgroundEnabled",
  },
```

And updated `PlayFeaturesControls.test.tsx` fixtures: added `"starfield_background_enabled"` to `ALL_FLAG_KEYS`, bumped `toHaveLength(3)` → `4` and the title "three entries" → "four entries", and added a `setStarfieldBackgroundEnabled: vi.fn(...)` stub to `StubApi` + `buildStubApi()`.
