<!-- BAD OUTPUT — DEFECT: only the integration test list updated; unit FLAGS fixture (7b) omitted -->
<!-- Violates SKILL.md lines 270-281: "Two test files must be edited or they red-fail." -->
...
The integration lock test now agrees across all three declarations (backend defaults, shared TS, kiosk paths) and passes. Default is `True`, so `test_all_surviving_flags_default_true` needs no change (left unchanged).
