<!-- BAD OUTPUT — DEFECT: flag key lacks the mandatory _enabled suffix -->
<!-- Violates SKILL.md line 22: "requires the literal `_enabled` suffix for boolean flags" -->
<!-- GOOD OUTPUT — reference for add-parent-feature-flag -->
...
# Wired `starfield_background` end-to-end

## Step 0 — Derived names

| Token | Value |
|---|---|
| `<FLAG_SNAKE>` | `starfield_background` (operator's name used as-is) |
| `<flag-kebab>` | `starfield-background` |
| `<FLAG_UPPER>` | `STARFIELD_BACKGROUND` |
| `<FlagPascal>` | `StarfieldBackground` |
| `<Default Bool>` | `True` (operator gave no default; convention default) |

## Step 1 — Backend core module (created `src/toybox/core/starfield_background.py`)

Cloned from `clickable_words_enabled.py`; storage delegated to `FeatureFlagSetting`.

```python
"""Household-scoped ``starfield_background`` flag.
...
STARFIELD_BACKGROUND_DEFAULT: bool = True

_SETTING = FeatureFlagSetting(
    key="starfield_background",
    default=STARFIELD_BACKGROUND_DEFAULT,
)


def get(conn: sqlite3.Connection) -> bool:
    """Return the persisted ``starfield_background`` flag (default ``True``)."""
...
__all__ = ["STARFIELD_BACKGROUND_DEFAULT", "get", "set"]
```

Did NOT re-export from `core/__init__.py`; imported only `FeatureFlagSetting`.

## Step 2 — Per-flag settings router (created `src/toybox/api/starfield_background_settings.py`)

GET unauthenticated; PUT parent-scoped; no If-Match/409; no WS broadcast; 422 detail `invalid_starfield_background`; `check_same_thread=False` kept.

```python
"""``starfield_background`` setting HTTP API.
...
from ..core import starfield_background as core_starfield_background
from ..core.auth import TokenScope
from ..db import connect, resolve_db_path
from .auth_dep import RequireScope

router = APIRouter(prefix="/api/settings", tags=["starfield_background_settings"])
...
class StarfieldBackgroundResponse(BaseModel):
    """Wire shape for ``GET`` and ``PUT`` ``/api/settings/starfield-background``."""

    value: bool


class StarfieldBackgroundUpdate(BaseModel):
    """Request body for ``PUT /api/settings/starfield-background``."""

    value: bool


@router.get("/starfield-background", response_model=StarfieldBackgroundResponse)
def get_starfield_background_endpoint(
    conn: Annotated[sqlite3.Connection, Depends(get_db)],
) -> StarfieldBackgroundResponse:
    """Return the persisted ``starfield_background`` flag."""
    value = core_starfield_background.get(conn)
    return StarfieldBackgroundResponse(value=value)


@router.put("/starfield-background", response_model=StarfieldBackgroundResponse)
def put_starfield_background_endpoint(
    body: StarfieldBackgroundUpdate,
    conn: Annotated[sqlite3.Connection, Depends(get_db)],
    _: Annotated[Any, Depends(RequireScope({TokenScope.parent}))],
) -> StarfieldBackgroundResponse:
    """Persist ``value`` and return the canonical bool."""
    try:
        new_value = core_starfield_background.set(conn, body.value)
    except ValueError as exc:
        raise HTTPException(
            status_code=422,
            detail={"error": "invalid_starfield_background"},
        ) from exc
    return StarfieldBackgroundResponse(value=new_value)


__all__ = [
    "StarfieldBackgroundResponse",
    "StarfieldBackgroundUpdate",
    "get_db",
    "router",
]
```
...
```python
    ...
    app.include_router(starfield_background_settings_router)
```

## Step 4 — Migration seed (created `src/toybox/db/migrations/0022_starfield_background.sql`)

Verified highest existing version is `0021_drop_deprecated_play_flags.sql` → new file is `0022`.

```sql
-- Seed the default row for the ``starfield_background`` parent feature flag.
-- Forward-only; INSERT OR IGNORE is idempotent (re-run is a no-op when
-- the operator has already set a value). Value mirrors
-- STARFIELD_BACKGROUND_DEFAULT in src/toybox/core/starfield_background.py.
--
...
INSERT OR IGNORE INTO settings (key, value) VALUES ('starfield_background', 'true');
```
...
> = {
  ...
  starfield_background: "/api/settings/starfield-background",
};
```
...
```python
        "starfield_background": (starfield_background.STARFIELD_BACKGROUND_DEFAULT),
```

(7b) `tests/unit/core/test_phase_k_feature_flags.py` — imported the module and added to the `FLAGS` fixture:

```python
    FlagFixture(starfield_background, "starfield_background", True),
```
...
(8a) `frontend/src/parent/api.ts` — added the `getStarfieldBackground` / `setStarfieldBackground` pair:

```ts
  async getStarfieldBackground(
    opts: RequestOptions = {},
  ): Promise<FeatureFlagResponse> {
    return this.request<FeatureFlagResponse>(
      "/api/settings/starfield-background",
      { method: "GET", signal: opts.signal },
    );
  }

  async setStarfieldBackground(
    value: boolean,
    opts: RequestOptions = {},
  ): Promise<FeatureFlagResponse> {
    return this.request<FeatureFlagResponse>(
...
```ts
        starfieldBackgroundResult,
```
```ts
        api.getStarfieldBackground({ signal: aborter.signal }),
```
```ts
        ["starfield_background", starfieldBackgroundResult],
```
...
type FlagSetterName =
  ...
  | "setStarfieldBackground";
```
```ts
  {
    key: "starfield_background",
    label: "Starfield background",
    hint: "Animated starfield backdrop on the kiosk. When off, the flat persona background renders instead.",
    setter: "setStarfieldBackground",
  },
```

And updated `PlayFeaturesControls.test.tsx` fixtures: added `"starfield_background"` to `ALL_FLAG_KEYS`, bumped `toHaveLength(3)` → `4` and the title "three entries" → "four entries", and added a `setStarfieldBackground: vi.fn(...)` stub to `StubApi` + `buildStubApi()`.
