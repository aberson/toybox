<!-- BAD OUTPUT — DEFECT: claims feature_flags.ts was regenerated via tools/gen_types_ts.py (it is NOT codegen) -->
<!-- Violates SKILL.md line 237: "it is NOT codegen; tools/gen_types_ts.py knows nothing about it" -->
...
Added the union member + defaults entry, then regenerated the file by running `python tools/gen_types_ts.py` so the pydantic→typescript codegen stays the source of truth. Flat literals preserved.
