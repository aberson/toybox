"""Household-scoped game-complexity dial setting.

Phase W Step W1 true-stub. Stores ``settings.game_complexity`` (TEXT,
...
tune how complex generated activities are.
"""
...
GAME_COMPLEXITY_VALID: frozenset[str] = frozenset({"low", "medium", "high"})
DEFAULT: str = "medium"

_SETTINGS_KEY = "game_complexity"


def get_game_complexity(conn: sqlite3.Connection) -> str:
    """Return the persisted game-complexity dial, defaulting to "medium".
    ...
    2. The stored value is not in :data:`GAME_COMPLEXITY_VALID`
    ...
    if raw not in GAME_COMPLEXITY_VALID:
...
def set_game_complexity(conn: sqlite3.Connection, value: str) -> str:
    """Persist ``value`` and return the canonical string.

    Raises :class:`ValueError` when ``value`` is not in
    :data:`GAME_COMPLEXITY_VALID`. The API layer translates this into
    HTTP 422 with the full canonical list in the error body.
    """
    if value not in GAME_COMPLEXITY_VALID:
        raise ValueError(f"invalid game complexity: {value!r}")
    with conn:
        conn.execute(
