...
// Phase L Step L7: reward ingest wire shapes. Mirror the Pydantic
// models in src/toybox/api/rewards.py (L2).
//
// ``Reward`` is the shape returned on confirm/list/get/patch/delete.
// ``image_path`` is the relative path under ``data/images/rewards/``;
// ``imageUrl()`` converts it to a fetchable URL the same way it does
// for toys. ``Animation`` is imported at the top of this module from
// shared/types.ts (the autogenerated re-export of the Python enum).
...
// Response from POST /api/rewards/upload. No vision suggestion phase —
// rewards are simple uploads (no Claude vision call).
...
// Body of the 409 returned by POST /api/rewards/upload when the SHA-256
// hash matches an existing non-archived reward. Surfaced in the UI as a
// "this image is already in your library" banner.
export interface RewardImageExistsDetail {
  code: "image_already_exists";
  existing_reward: Reward;
}

// Body for POST /api/rewards (confirm). ``staging_key`` comes from the
// upload response. ``animation`` is required (the form's segmented
// control defaults to ``shine``).
export interface RewardConfirmRequest {
  staging_key: string;
  display_name: string;
  tags: string[];
  animation: Animation;
...
// Body for PATCH /api/rewards/{id}. All fields optional; only fields
// present in the body are written. Omitting a field leaves the existing
// value untouched; explicit ``null`` is rejected server-side (422).
...
export class ApiClient {
  ...
  // Phase L Step L7: reward ingest pipeline. Mirrors the toy upload →
  // confirm shape, but with no Claude vision step (rewards are simple
  // uploads: parent picks image + display name + tags + animation +
  // active toggle). Backend at src/toybox/api/rewards.py (L2).
  async uploadReward(
    file: File,
    opts: RequestOptions = {},
  ): Promise<RewardUploadResponse> {
    const headers = new Headers();
    const token = this.getToken();
    if (token !== null) {
      headers.set("X-Toybox-Token", token);
    }
    // ``Content-Type: multipart/form-data; boundary=...`` is set by the
    // browser when ``body`` is a ``FormData``; setting it manually
    // would clobber the boundary token.
    const form = new FormData();
    form.append("file", file);
    const resp = await this.fetchImpl(this.baseUrl + "/api/rewards/upload", {
      method: "POST",
      headers,
      body: form,
      signal: opts.signal,
    ...
    return body as RewardUploadResponse;
  }
  ...
  async listRewards(
    ...
    return this.request<RewardListResponse>("/api/rewards", {
      method: "GET",
      signal: opts.signal,
    });
  }

  async getReward(id: string, opts: RequestOptions = {}): Promise<Reward> {
    return this.request<Reward>(`/api/rewards/${encodeURIComponent(id)}`, {
      method: "GET",
      signal: opts.signal,
    });
  }
  ...
  // Soft delete on the backend (sets ``archived = 1`` and returns the
  // now-archived row). Named ``archiveReward`` to mirror ``archiveToy``
  // and to read honestly at the call site.
  async archiveReward(
    id: string,
    opts: RequestOptions = {},
  ): Promise<Reward> {
...
// Phase L Step L7: pull the image_already_exists detail off a 409
// ApiError raised by POST /api/rewards/upload. Mirrors
// ``extractToyImageExistsDetail`` but keyed on ``existing_reward`` so
// the RewardIngest UI can surface a "this image is already in your
// library" banner with the existing reward's display name.
export function extractRewardImageExistsDetail(
  err: unknown,
): RewardImageExistsDetail | null {
  if (!(err instanceof ApiError) || err.status !== 409) return null;
  const body = err.body;
  if (typeof body !== "object" || body === null) return null;
  const rec = body as Record<string, unknown>;
  ...
  if (
    c["code"] === "image_already_exists" &&
    typeof c["existing_reward"] === "object" &&
    c["existing_reward"] !== null
  ) {
    return {
      code: "image_already_exists",
      existing_reward: c["existing_reward"] as Reward,
    };
  }
  return null;
}
