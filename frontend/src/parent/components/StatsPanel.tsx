// Phase H step H5: Stats sub-tab panel. Hosts the metrics-snapshot
// dashboard half of the pre-H5 OperatorTab — the operator stats view
// without any toggles. Auto-refresh strategy is unchanged from
// OperatorTab:
// 1. On mount, GET /api/metrics for the first paint.
// 2. While mounted, subscribe to ``metrics`` ws envelopes via the
//    optional ``subscribeToMetrics`` prop — each envelope replaces the
//    snapshot in state.
// 3. If the ws stalls (no envelopes within ``stalenessThresholdMs``),
//    REST-poll every ``pollIntervalMs``.
...
// off the ``stats-panel`` data-testid so the rules don't leak into the
// rest of the parent UI.
const SCOPED_CSS = `
[data-testid="stats-panel"] table {
  width: 100%;
  border-collapse: collapse;
  font-size: 11px;
}
[data-testid="stats-panel"] td {
  padding: 2px 6px;
  vertical-align: top;
}
[data-testid="stats-panel"] td:first-child {
  color: #6b7280;
  white-space: nowrap;
}
[data-testid="stats-panel"] td:nth-child(2) {
  font-variant-numeric: tabular-nums;
  word-break: break-word;
}
[data-testid="stats-panel"] h2 {
  font-size: 16px;
  margin: 0 0 4px 0;
}
`;
...
export function StatsPanel(props: StatsPanelProps): JSX.Element {
  ...
  if (snapshot === null) {
    return (
      <section data-testid="stats-panel" style={{ padding: 12, fontSize: 12 }}>
        <style>{SCOPED_CSS}</style>
        <h2>Stats</h2>
        {error !== null ? (
          <div data-testid="stats-error" role="alert" style={{ color: "#a00" }}>
            {error}
          </div>
        ) : (
          <div data-testid="stats-loading">loading metrics…</div>
        )}
      </section>
    );
  }
  ...
  return (
    <section data-testid="stats-panel" style={{ padding: 12, fontSize: 12 }}>
      ...
      <div
        data-testid="stats-ws-subscribers"
