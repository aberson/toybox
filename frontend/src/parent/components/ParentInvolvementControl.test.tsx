// Phase W Step W1 — ParentInvolvementControl component tests.
...
// - Clicking a different option fires setParentInvolvement PUT
...
function buildStubApi(): StubApi {
  return {
    setParentInvolvement: vi.fn(async (value: ParentInvolvement) => ({
      value,
    })) as Mock,
  };
}
...
describe("ParentInvolvementControl", () => {
  it("renders all 3 option buttons with the correct labels + testids", () => {
    ...
    expect(screen.getByTestId("parent-involvement-low").textContent).toBe(
      "Low",
    );
    expect(screen.getByTestId("parent-involvement-medium").textContent).toBe(
      "Medium",
    );
    expect(screen.getByTestId("parent-involvement-high").textContent).toBe(
      "High",
    );
  });
  ...
  it("currentValue=medium shows the medium button as pressed and the others as not", () => {
    ...
    expect(
      screen
        .getByTestId("parent-involvement-medium")
        .getAttribute("aria-pressed"),
    ).toBe("true");
    for (const value of ["low", "high"]) {
      expect(
        screen
          .getByTestId(`parent-involvement-${value}`)
          .getAttribute("aria-pressed"),
      ).toBe("false");
    }
  });

  it("clicking high calls setParentInvolvement(high) and onValueChanged(high)", async () => {
    const api = buildStubApi();
    ...
    fireEvent.click(screen.getByTestId("parent-involvement-high"));
    await waitFor(() => {
      expect(api.setParentInvolvement).toHaveBeenCalledWith(
        "high",
        expect.anything(),
      );
    });
    ...
    await waitFor(() => {
      expect(
        (
          screen.getByTestId("parent-involvement-high") as HTMLButtonElement
        ).disabled,
      ).toBe(false);
    });
  });
  ...
  it("during the in-flight PUT, all buttons are disabled and aria-pressed tracks the optimistic value", async () => {
    const api = buildStubApi();
    type Resolver = (value: { value: ParentInvolvement }) => void;
    const resolverRef: { current: Resolver | null } = { current: null };
    api.setParentInvolvement.mockImplementationOnce(
      () =>
        new Promise<{ value: ParentInvolvement }>((resolve) => {
    ...
    fireEvent.click(screen.getByTestId("parent-involvement-low"));
    await waitFor(() => {
      expect(
        (
          screen.getByTestId("parent-involvement-low") as HTMLButtonElement
        ).disabled,
      ).toBe(true);
    });
    ...
    expect(
      screen
        .getByTestId("parent-involvement-low")
        .getAttribute("aria-pressed"),
    ).toBe("true");
  ...
  it("PUT rejection surfaces an inline error and does NOT call onValueChanged, reverts to currentValue", async () => {
    const api = buildStubApi();
    api.setParentInvolvement.mockRejectedValueOnce(new Error("dial boom"));
    const onValueChanged = vi.fn();
    render(
      <ParentInvolvementControl
        api={api as unknown as ApiClient}
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("parent-involvement-error").textContent,
      ).toContain("dial boom");
    });
  ...
  it("does not fire a second PUT when the operator re-clicks during an in-flight request", async () => {
    ...
    fireEvent.click(screen.getByTestId("parent-involvement-low"));
    fireEvent.click(screen.getByTestId("parent-involvement-high"));
    expect(api.setParentInvolvement).toHaveBeenCalledTimes(1);
    resolverRef.current?.({ value: "low" });
    await waitFor(() => {
      expect(onValueChanged).toHaveBeenCalledWith("low");
    });
