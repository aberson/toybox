...
>
> **iPad UAT 11/12 PASS** at Phase M close (2026-05-18) — kiosk runs as a Guided Access app on a real iPad over Wi-Fi LAN. **2,670 pytest + 802 vitest** green at master `f878eb7` (2026-06-22); Phase W (play depth), Phase X (room import), and Phase Y (scene backdrops) are code-shipped and awaiting iPad UAT ([#223](https://github.com/aberson/toybox/issues/223) + Phase Y Manual UAT). Fully offline once whisper-small + persona avatars + element sprites are cached locally.
...
- **Offline catalog browse (Phase T, 2026-06-06)** — `GET /api/catalog` + `CatalogPanel` let the parent browse and filter the full template library (category/theme chips) with no backend AI.
- **Toy-action animation (Phase V, 2026-06-07)** — hybrid SVD-idle + CSS slot-entry sprites on the kiosk; `prefers-reduced-motion` respected. Replaced the abandoned Phase U AnimateDiff approach.
