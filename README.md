...
**Phase K substrate complete (K1-K15) 2026-05-15.** Issues #114–#128 closed (15 of 18 K-step issues). 10-role taxonomy + slot-fill engine + proposed-only recast endpoint; ~50-entry joke corpus + song-corpus manifest + Coqui render script (`scripts/generate_song_corpus.py`); kiosk Web Speech TTS substrate + per-persona voice profile + click-to-read (word taps + watermarked Read Me button); 4 interjection delivery surfaces (standalone-intent activities, theme-tagged embedded, endings, parent-insert + persona/role spontaneity), all sharing a single `build_interjection_step` source of truth; 8 parent feature flags (`jokes_enabled`, `songs_enabled`, 4 surface toggles, 2 kiosk toggles) wired through migration 0015 + per-setting modules + `PlayFeaturesControls`. K16 (overnight 4-agent template backfill), K17 (end-to-end smoke gate, requires operator-rendered Coqui `.mp3` corpus), K18 (M1 iPad operator UAT) deferred to a fresh session. 1819 backend pytest + 515 frontend vitest passing. 0 type errors, 0 lint violations. Phase J (autonomous play queue, J1-J12) shipped 2026-05-14 in the same release window — operator UAT PASS.
...
- **Phase K substrate** (2026-05-15): roles + songs + jokes + voice — K1-K15 shipped this commit; K16-K18 deferred (see Build status above).
...
- **Phase K closeout**: K16 overnight 4-agent template backfill (200 templates → role-aware + themed + ending steps), K17 end-to-end smoke gate (requires `pip install TTS && python scripts/generate_song_corpus.py` to populate `data/songs/audio/*.mp3` first), K18 (M1) iPad operator UAT.
