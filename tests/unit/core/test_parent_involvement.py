"""Unit coverage for :mod:`toybox.core.parent_involvement`."""
...
from toybox.core.parent_involvement import (
    DEFAULT,
    PARENT_INVOLVEMENT_VALID,
)
from toybox.core.parent_involvement import (
    get_parent_involvement as get,
)
from toybox.core.parent_involvement import (
    set_parent_involvement as set,
)
...
def test_get_default_when_row_missing(
    ...
    db.execute("DELETE FROM settings WHERE key = 'parent_involvement'")
...
def test_get_corrupt_value_logs_warning(
    ...
    with db:
        db.execute(
            ...
            ("parent_involvement", stored_value),
        )

    # Act
    with caplog.at_level(logging.WARNING, logger="toybox.core.parent_involvement"):
        value = get(db)
    ...
    warnings = [
        ...
        if rec.levelno == logging.WARNING and rec.name == "toybox.core.parent_involvement"
    ]
...
def test_set_round_trips_every_value(
    ...
    for value in sorted(PARENT_INVOLVEMENT_VALID):
        result = set(db, value)
        assert result == value
        assert get(db) == value
...
def test_set_rejects_invalid_values(
    ...
    for invalid in ["", "LOW", "extreme", "med", "5"]:
        with pytest.raises(ValueError, match=r"invalid parent involvement"):
            set(db, invalid)
