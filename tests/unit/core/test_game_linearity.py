"""Unit coverage for :mod:`toybox.core.game_linearity`."""
...
from toybox.core.game_linearity import (
    DEFAULT,
    GAME_LINEARITY_VALID,
)
from toybox.core.game_linearity import (
    get_game_linearity as get,
)
from toybox.core.game_linearity import (
    set_game_linearity as set,
)
...
def test_default_is_nonlinear() -> None:
    """The default dial value is ``nonlinear`` (byte-identical pre-W2 behavior)."""
    assert DEFAULT == "nonlinear"
...
def test_get_default_when_row_missing(
    ...
    # Arrange — migration 0025 seeded the row; remove it to simulate
    # legacy / hand-edited DBs that predate the seed.
    db.execute("DELETE FROM settings WHERE key = 'game_linearity'")
...
def test_get_corrupt_value_logs_warning(
    ...
    with db:
        db.execute(
            ...
            ("game_linearity", stored_value),
        )

    # Act
    with caplog.at_level(logging.WARNING, logger="toybox.core.game_linearity"):
        value = get(db)
    ...
    warnings = [
        ...
        if rec.levelno == logging.WARNING and rec.name == "toybox.core.game_linearity"
    ]
...
def test_set_round_trips_every_value(
    ...
    for value in sorted(GAME_LINEARITY_VALID):
        result = set(db, value)
        assert result == value
        assert get(db) == value
...
def test_set_rejects_invalid_values(
    ...
    for invalid in ["", "LINEAR", "branching", "non-linear", "5"]:
        with pytest.raises(ValueError, match=r"invalid game linearity"):
            set(db, invalid)
