...
async def test_template_cache_reused_across_calls(
    templates_root: Path,
) -> None:
    """A second call with the same slot doesn't re-read the template PNG.

    Pinned because the spec says "cache after first read; per-process
    module-level cache OK". After two calls, the module-level cache
    must contain exactly one entry for the (templates_dir, slot) key.
    """
    ref = _make_reference_jpeg()
    await composite_action(ref, slot="idle", seed=1, ctx=_ctx())
    await composite_action(ref, slot="idle", seed=2, ctx=_ctx())
    # Internal-state introspection: the cache key is
    # (str(templates_dir), slot).
    cache = composite._TEMPLATE_CACHE
    keys = [k for k in cache if k[1] == "idle"]
    assert len(keys) == 1, cache


async def test_lazy_import_contract_for_composite() -> None:
    """``import toybox.image_gen.composite`` does NOT load rembg / PIL.

    Mirrors :mod:`tests.unit.image_gen.test_lazy_imports` — the
    composite module's heavy deps (rembg, PIL) live inside
    ``_composite_sync``. A top-level ``import`` would fail this.
    """
    import subprocess

    snippet = (
        "import sys\n"
        "import toybox.image_gen.composite\n"
        "forbidden = {'rembg', 'torch', 'diffusers', 'transformers'}\n"
