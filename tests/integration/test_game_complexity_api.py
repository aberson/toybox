"""Integration coverage for the /api/settings/game-complexity endpoints.
...
from toybox.api.game_complexity_settings import get_db
from toybox.app import create_app
from toybox.core.auth import TokenScope, issue_token
from toybox.core.game_complexity import GAME_COMPLEXITY_VALID
...
def test_get_returns_default_when_seed_absent(
    ...
    try:
        with conn:
            conn.execute("DELETE FROM settings WHERE key = 'game_complexity'")
    finally:
        conn.close()

    # Act
    response = client.get("/api/settings/game-complexity")
...
def test_put_round_trip_representative_values(
    ...
    get_response = client.get("/api/settings/game-complexity")
