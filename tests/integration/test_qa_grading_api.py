"""Integration coverage for the /api/settings/qa-grading endpoints.

GET is unauthenticated (household read, mirrors the parent-involvement
...
hard-coding it. Phase W Step W3.
"""
...
from toybox.api.qa_grading_settings import get_db
from toybox.app import create_app
from toybox.core.auth import TokenScope, issue_token
from toybox.core.qa_grading import QA_GRADING_VALID
...
def test_get_returns_default_when_seed_absent(
    ...
    """GET returns off even when the seed row has been deleted (legacy DB)."""
    conn = connect(db_path)
    try:
        with conn:
            conn.execute("DELETE FROM settings WHERE key = 'qa_grading'")
    finally:
        conn.close()

    response = client.get("/api/settings/qa-grading")
...
@pytest.mark.parametrize("value", ["lenient", "strict"], ids=["lenient", "strict"])
def test_put_round_trip_representative_values(
    ...
    get_response = client.get("/api/settings/qa-grading")
...
def test_put_without_token_returns_401(client: TestClient) -> None:
    ...
    response = client.put("/api/settings/qa-grading", json={"value": "strict"})
