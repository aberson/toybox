...
| [phase-y-scene-backdrops-plan.md](plan/awaiting-uat/phase-y-scene-backdrops-plan.md) — scene backdrops behind the kiosk step card + per-child scene selection (activates the Children tab `interests` signal) | CODE-SHIPPED 2026-06-22 at master `f878eb7` — Y1-Y8 merged (migration 0030 `activities.scene_id`; `scene_catalog` single source of truth; offline `batch_scenes.py` + no-IPA `generate_scene`; template `scene_id` → child-interest → default resolver chain; `ActivityResponse.scene_url` denormalized + codegen; kiosk full-bleed backdrop layer; per-call IP-Adapter-scale override infra). Umbrella [#264](https://github.com/aberson/toybox/issues/264), steps [#265-#266, #268-#273](https://github.com/aberson/toybox/issues/264) closed. Operator steps Y2-run ([#267](https://github.com/aberson/toybox/issues/267) — GPU scene-library render) + Y9 ([#274](https://github.com/aberson/toybox/issues/274) — iPad backdrop UAT) are the plan's Manual UAT bundle (M1/M2). 2,670 pytest + 802 vitest. |
...
| Y — scene backdrops + per-child scene selection | [phase-y-scene-backdrops-plan.md](plan/awaiting-uat/phase-y-scene-backdrops-plan.md) | **M2** (#274 — iPad backdrop UAT, in the plan's Manual UAT bundle) | **M1** (#267 — `uv run python scripts/batch_scenes.py` with backend stopped → render + eyeball the scene library), **separate** |

When the #223 bundle passes, move the validated plans from `awaiting-uat/` to `archive/` and update their Status-table rows above. P/Q render/generator steps are tracked separately (not iPad UAT). Phase Y's Manual UAT (M1 render + M2 iPad) lives in its own plan doc's `## Manual UAT` section.
