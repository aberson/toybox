...
> - [`plan/awaiting-uat/`](plan/awaiting-uat/) — phases whose **code shipped but operator UAT is still open** (O, P, Q, R, S, T, V, W, X, Z). See the [Awaiting UAT](#awaiting-uat) section.
...
| [phase-y-scene-backdrops-plan.md](plan/archive/phase-y-scene-backdrops-plan.md) — scene backdrops behind the kiosk step card + per-child scene selection (activates the Children tab `interests` signal) | ✅ COMPLETE 2026-06-23 — Y1-Y8 CODE-SHIPPED 2026-06-22 at master `f878eb7` (migration 0030 `activities.scene_id`; `scene_catalog` single source of truth; offline `batch_scenes.py` + no-IPA `generate_scene`; template `scene_id` → child-interest → default resolver chain; `ActivityResponse.scene_url` denormalized + codegen; kiosk full-bleed backdrop layer; per-call IP-Adapter-scale override infra). Manual UAT M1 ([#267](https://github.com/aberson/toybox/issues/267) — GPU scene-library render) + M2 ([#274](https://github.com/aberson/toybox/issues/274) — iPad backdrop UAT) **PASS** (run doc [2026-06-23-phase-y-uat.md](runs/2026-06-23-phase-y-uat.md)). Umbrella [#264](https://github.com/aberson/toybox/issues/264) + all step issues closed. 2,670 pytest + 802 vitest. |
| [phase-z-persona-voices-plan.md](plan/awaiting-uat/phase-z-persona-voices-plan.md) — persona voices: server-rendered Kokoro-82M neural TTS clips + voice-profile wire-through + sentence-boundary fallback truncation | CODE-SHIPPED 2026-07-03 at master `41e04fe` — Z1-Z7-prep merged (migration 0031 `neural_voice_enabled`; `toybox.tts` engine/cache/worker + `/api/static/tts` mount; `spoken_audio_url` wire shape at 6 producer sites; kiosk `clip-audio.ts` clip-first playback with Web Speech fallback; `scripts/batch_tts_audition.py`); umbrella [#2](https://github.com/aberson/toybox/issues/2), steps [#3-#9](https://github.com/aberson/toybox/issues/2) closed. Operator span pending: the plan's `## Manual UAT` M1 voice audition ([#10](https://github.com/aberson/toybox/issues/10)) → M2 real-engine smoke ([#11](https://github.com/aberson/toybox/issues/11)) → M3 iPad UAT ([#12](https://github.com/aberson/toybox/issues/12)) — **NOT folded into bundle #223** (needs the `tts` extra + model download first). 2,788 pytest + 897 vitest. |
...
| Z — persona voices (Kokoro TTS) | [phase-z-persona-voices-plan.md](plan/awaiting-uat/phase-z-persona-voices-plan.md) | **own `## Manual UAT` M1→M2→M3 ([#10](https://github.com/aberson/toybox/issues/10)-[#12](https://github.com/aberson/toybox/issues/12)), separate from #223** | M1 prereqs: `uv sync --extra tts` + `python -m toybox.tts --download` + audition listen/sign-off |
...
When the #223 bundle passes, move the validated plans from `awaiting-uat/` to `archive/` and update their Status-table rows above. P/Q render/generator steps are tracked separately (not iPad UAT). (Phase Y's Manual UAT — M1 render + M2 iPad — passed 2026-06-23; its plan is now archived.)
