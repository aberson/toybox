...
- **Depends on:** M1, M3.
- **Status:** DONE (2026-05-18) iter 1/3. 30 templates exactly matching the plan distribution (4+4+4+3+5+3+3+4). All 8 family-name slugs used verbatim in narration; 0 banned synonyms. Each template carries 1 fork point with 2-4 choices ("2-4 forks" in §5.5 read as "choices per fork", matching existing Phase G shape). element_id on entry step only across all 30. Code review: 0 HIGH / 1 MED (plan-language ambiguity, not a real issue) / 2 LOW (stale docstring fixed inline). Hand-authored via `scripts/generate_family_pretend_templates.py` (2315 LOC, mostly Python literals). request_play.json 250 → 280. 1972 backend pytest pass / 3 skipped. mypy + ruff clean.
...
- **Depends on:** M1, M3.
- **Status:** DONE (2026-05-18) iter 1/3 + inline {guide_mentor} fix. 15 templates exactly. Phase coverage: 5 gas (he-2, o-8, h-1, ne-10, ar-18), 3 liquid (hg-80, water-via-o-8, n-7 liquid nitrogen), 5 solid (au-79, fe-26, c-6, na-11, li-3), 2 famous-metals (ag-47, cu-29). All age-appropriate framing — dangerous elements (mercury, liquid nitrogen) explicitly framed as "magic bubble keeps us safe"; no existential-emptiness language. Verifier caught HIGH bug iter 1: 14/15 templates had a literal "Iridia" leak in fork-step text (alongside correct `{guide_mentor}` placeholder elsewhere). Fixed via regex replace in the generator + regeneration — 0 literal-name leaks remain. recommended_themes=["adventure","magic"] all; element_id on entry step only; required_roles=["guide_mentor"]. Hand-authored via `scripts/generate_shrink_journey_templates.py` (794 LOC). request_story.json 250 → 265. 1972 backend pytest pass / 3 skipped (was 1972 + 2; +0 new tests). mypy + ruff clean.
...
- **Status:** SPLIT 2026-05-18 per `.claude/rules/plan-and-issue-flow.md` § "Operator-type steps must not produce code artifacts" (symmetric inverse — `Type: code` step had an operator-run-and-confirm in Done-when). Two subtasks:
  - **M7a (code, DONE 2026-05-18):** 25 manifest entries appended via `scripts/generate_element_song_manifest.py`. load_songs() count 50 → 75. 10 family songs (one per Family enum value) + 15 individual popular elements (gold, silver, iron, helium, oxygen, hydrogen, neon, mercury, copper, uranium, sodium, calcium, carbon, nitrogen, chlorine). Themes 11 silly + 14 music. age_band="3-5", persona_compat=["periodic_table","all"]. Lyrics spot-checked clean (chemistry-accurate, no dangerous-interaction encouragement, kid-friendly vocabulary). 1972 backend pytest pass, mypy + ruff clean.
...
- **Status:** DONE (2026-05-18) iter 1/3. 87-row grep audit completed. Fixes: 3 test sites with full-set enumeration patterns (test_taxonomies snapshot 12→13, test_joke_corpus + test_song_corpus completeness tests both exempt `Theme.feelings` until M9-M12 lands content). Frontend has NO `Theme` coupling — banned-themes is free-form, codegen doesn't emit Theme union. topic_extract.py's keyword dict gets `feelings` mapping in M9-M12 alongside the SEL content (vote-based extractor silently safe with zero-keyword themes). 4 new tests in `tests/unit/test_themes.py` covering `is`-identity, display-map round-trip, completeness snapshot, display-map vs enum parity. Backend 1976 pass / 3 skipped (was 1972 + 2; +4 net). Frontend 598 (unchanged). mypy + ruff + tsc + eslint clean. Skipped 4-way review: the grep audit IS the review for single-source-of-truth changes.
...
- **Depends on:** M8.
- **Status:** DONE (2026-05-18) iter 1/3. 20 templates each with the two-act structure (act 1 from {friend} POV, act 2 replays from {frenemy} POV). Act-2 reveal types vary across 5 categories: fixing/helping, timing mistake, misread state, didn't-notice/didn't-see, positive misread. All 5 spot-checked templates pass the load-bearing M10 gate (act 2 reveals NEW info, frenemy never villainized, has a real reason). 8 steps each (within 6-10 range). 2 role placeholders ({friend} + {frenemy}), 2 required_roles → K3.2 gate 2≤2. Persona-name leak grep: 0 hits. `scripts/generate_perspective_taking_templates.py` (2327 LOC). request_play.json 280 → 300. Backend 1976 pass / 3 skipped (flaky ws tests pass on rerun). mypy + ruff clean. UAT-config gap (frenemy-eligible toy in install pool) carried as M14 operator pre-check per plan §8.
...
- **Depends on:** M8.
- **Status:** DONE (2026-05-18) iter 1/3. 13 play + 12 activity = 25 templates exactly. Resolution-strategy coverage: split it (16), find a substitute (22), take turns (24), one offers/one accepts (22). 12 templates use all 4 strategies; 13 use 3 strategies (skipping the strategy that didn't fit the scenario, e.g. "rules of a new game" has no clean "one offers" framing). 84 total fork branches across the 25 templates, each with a feelings check-in (verified programmatically). Spot-checked 5: feelings vocabulary diverse (sharing-proud, glad-they-thought-of-it, patient, generous-wistful, clever, fair, helpful, satisfied, grateful), no "and they all hugged" boilerplate. Persona-name leak grep: 0 hits. Dev solved a placeholder-shape issue (two `{friend}` placeholders would resolve to one toy; `{friend2}` doesn't match the placeholder regex) by using `{friend}` + `{frenemy}` as slot keys with `required_roles=["friend"], optional_roles=["frenemy"]` — frenemy is just a slot label for distinct toy allocation, NOT adversarial framing in narration. `scripts/generate_conflict_resolution_templates.py` (2090 LOC). request_play 300 → 313, request_activity 368 → 380. Backend 1976 pass / 3 skipped. mypy + ruff clean.
...
- **Depends on:** M8.
- **Status:** DONE (2026-05-18) iter 1/3. 15 templates exactly. **All 45 fork branches** (15 templates × 3 forks each) follow the "first try fails, second try works" recovery pattern — exceeds the plan's "at least one fork per template" floor. Programmatically verified by walking `rupture → choices → *_path → *_second` chains. Strategy distribution: apologize (15 universal), feelings (11), offer (11), fix (8). 3 strategies per template (no template uses all 4). Each `*_path` step has an explicit reason it doesn't land yet (too quick, too generic, wrong order, felt like brush-off, "just a joke"); each `*_second` step names what changed (added specificity, invited in, different angle, persistence + waiting). NO "instant forgiveness" shortcuts. 8 steps each (1 rupture + 6 fork-branches + 1 end), high end of plan's 5-8 range. Inherits M11's frenemy-as-slot-key pattern. `scripts/generate_friendship_repair_templates.py` (1385 LOC). request_play.json 313 → 328. Persona-name leak grep: 0 hits. Backend 1976 pass / 3 skipped. mypy + ruff clean.
