...
Other phase plans referenced: [`phase-m-plan.md`](phase-m-plan.md) (immediate predecessor, defines M1-M14 and the element corpus + ElementCard), [`phase-l-plan.md`](phase-l-plan.md) (reward matcher + song corpus shape), [`phase-o-plan.md`](../awaiting-uat/phase-o-plan.md) (parent UX 5-tab refresh, consumes Phase N's `template_type`).
...
| **Phase O** | Parent UX 5-tab refresh — consumes Phase N's `template_type` field as the "Elements" categorization signal. Plan at [`phase-o-plan.md`](../awaiting-uat/phase-o-plan.md). |
...
- **Depends on:** N1.
- **Status:** DONE (2026-05-18) — shipped at `1b3e356` with 15 unit tests (full suite 2044 passing). Pattern matches sibling helpers (`rng: random.Random`, sort-by-id before `rng.choice`, ValueError on empty pool). Caught a stale §2 plan claim — only 15 of 118 entries are age_band 3-5 (not all 118). `na-11` and `ca-20` raise from `peer_in_family` by design; N4 generator must handle (fall back to story_seed_hooks).
