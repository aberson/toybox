...
> **Scope:** Phase F build plan — child-side toy-action sprites (post-v1). Carries the per-step `**Problem:**/**Type:**/**Issue:**/**Flags:**/**Status:**/**Done when:**` shape that `/build-phase` parses. Sequenced independently of [phase-e.md](phase-e.md) — Phase E and Phase F share migrations 0004/0005 numerically; whichever ships first claims those numbers and the other renumbers at build time (see §"Risks and open questions"). Top-level overview is in [../plan.md](../plan.md).
...
- **Issue:** #48
- **Flags:** --reviewers code
- **Status:** DONE (2026-05-06) — merged in 4e2d443. 8 files / 2370 insertions. Single-worker `image_gen/worker.py` (1083 lines) with dual-supersede semantics (enqueue-time + run-time via seed-mismatch fallback), restart recovery sweep covering BOTH `queued` AND `running` rows (closes the orphan-on-DB-write-before-queue-put leak), cancellation cleanup via `try/except CancelledError` writing `failed("interrupted by shutdown")` before re-raising, atomic enqueue via single-connection transaction, per-pipeline breaker that distinguishes data errors from pipeline errors. WS `Topic.toy_actions` registered parent-scope-only. Lifespan composed via `AsyncExitStack` into both `_metrics_lifespan` and `_build_smoke_lifespan`. 2 dev iterations + 1 continuation (iter-2 hit an API overload mid-flight; the production fixes landed pre-error and a tight follow-up agent added the 4 regression tests). Quality gates green on master: ruff clean, mypy strict 84 source files, pytest 1093 passed + 2 skipped.
...
- **Issue:** #49
- **Flags:** --reviewers code
- **Status:** DONE (2026-05-06) — merged in 23dce6a. 2 files / 930 insertions. Three new endpoints on `api/toys.py`: `GET /api/toys/{id}/actions` (returns 10-row list + capability info; 200 even when disabled per graceful-degradation invariant), `POST /api/toys/{id}/actions/regenerate` (10 slots), `POST /api/toys/{id}/actions/{slot}/regenerate` (single slot, distinct 404 code `slot_not_in_vocab`). Plus 503 `image_gen_worker_unavailable` envelope (deviation) distinguishing operational failure from operator-actionable 409 disabled. Toy-commit hook in `POST /api/toys` enqueues 10 jobs best-effort; mandatory integration test drives the production handler end-to-end (per `feedback_buildstep_require_integration_test.md`). UUIDv4 boundary validator maps malformed toy_id to 404. `post_confirm` flipped sync→async to await `worker.enqueue`. 25 integration tests; 1 dev iteration; quality gates green: ruff clean, mypy strict 84 source files, pytest 1118 passed + 2 skipped.
...
- **Issue:** #50
- **Flags:** --reviewers code
- **Status:** DONE (2026-05-06) — merged. 14 files / 817 insertions. Migration 0006 adds `action_slot TEXT` (nullable). Pydantic `ActivityStep.action_slot: Annotated[str | None, AfterValidator(_validate_action_slot)] = None` accepts None or any `ACTION_SLOTS` member; bad slot → ValueError → existing offline-fallback path. Generator (single-shot Claude + loop-mode) prompts updated with verbatim 10-key vocab + "default idle" instruction. All 4 shipped templates (boredom/request_play/request_activity/request_story) have hand-authored slots. Template loader rejects bad slots at boot. Schema kept lax — parser is single source of truth (deviation; documented inline). Existing fixtures stayed compatible thanks to default. WS `Topic.activity_state` payload picks up `action_slot` via existing `model_dump`. Integration test through `_do_propose` drives REST → DB → WS end-to-end. 16 new tests; 1 dev iteration. Quality gates green: ruff clean, mypy strict 84 source files, pytest 1135 passed + 2 skipped.
...
- **Issue:** #52
- **Flags:** --reviewers code --ui (canonical) — autonomous-build operating mode runs as `--reviewers code`; visual UI verification batches at F9 per [`feedback_autonomous_build_bundled_ui.md`](../../../../.claude/projects/c--Users-abero-dev/memory/feedback_autonomous_build_bundled_ui.md)
- **Status:** DONE (2026-05-06) — merged. New `parent/components/ToyActionGrid.tsx` (2×5 reusable; per-cell status branches; "regenerate all" + per-slot + N/10 done count; capability-disabled banner; hidden on archived toys). Wired into `ToyIngest.tsx` for both post-commit (form clears, grid renders below with "Saved. Generating action sprites..." notice) and toy-edit flows. `parent/api.ts` gains `listToyActions` / `regenerateAllActions` / `regenerateActionSlot`. `parent/ws.ts` subscribes to `Topic.toy_actions`. `parent/store.ts` reducer `applyToyActionEnvelope` with latest-wins-per-slot + field-level carry-over (seed/image_path) + malformed-payload allow-list defense. +17 vitest (9 grid + 8 reducer); frontend 227 passed.
