...
> **Scope:** Phase E build plan — local-model substrate, tool-loop refactor, non-linear gameplay (post-v1). Carries the per-step `**Problem:**/**Type:**/**Issue:**/**Flags:**/**Status:**` shape that `/build-phase` parses, plus phase-level goal, prerequisites, sequencing rationale, and open risks. Read this when working any Phase E step. Top-level overview is in [../plan.md](../plan.md).
...
| Telemetry volume gate for E3 | <200 tuples produces an under-determined SFT corpus; risk of overfitting to small-N quirks | Hard prerequisite ≥1 month real-use telemetry, ideally ≥200 tuples (per [phase-e.md prerequisite](phase-e.md)); Step 27 refuses to run if `labeled_events` row count below threshold | [phase-e.md](phase-e.md) |
