...
> **Scope:** Phase E build plan — local-model substrate + tool-loop refactor (post-v1). Carries the per-step `**Problem:**/**Type:**/**Issue:**/**Flags:**/**Status:**` shape that `/build-phase` parses, plus phase-level goal, prerequisites, sequencing rationale, and open risks. Read this when working any Phase E step. Top-level overview is in [../plan.md](../plan.md).
>
...
| Telemetry volume gate for E3 | <200 tuples produces an under-determined SFT corpus; risk of overfitting to small-N quirks | Hard prerequisite **≥50 rows passing the SFT filter** (data-volume floor; ideally ≥200 tuples). No calendar gate — the calendar minimum was dropped 2026-05-10. Step 27 refuses to run if `labeled_events` row count below threshold | [phase-e.md](phase-e.md) |
