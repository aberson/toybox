...
*Generated by /build-phase on 2026-07-03. Append-only; re-running the phase adds new items below, never modifies existing ones.*

### M1: Operator voice audition + casting sign-off
- **Source step:** Step Z7 (from this plan's §7)
- **Issue:** #10
- **Commands to run:**
  ```powershell
  uv sync --extra tts
  uv run python -m toybox.tts --download
  uv run python scripts/batch_tts_audition.py
  # listen to data/tts/audition/ in the printed manifest order (persona castings first)
  # to remap: edit voice_profile.neural_voice in src/toybox/personas/library/<persona>.json, then:
  #   uv run python -m toybox.db.migrate
  #   uv run python scripts/batch_tts_audition.py   # new persona_<id>_<voice>.wav appears without --force
  ```
- **What you're looking for:**

  | Check | Expected outcome |
  |---|---|
  | `--download` completes | `kokoro-v1.0.onnx` + `voices-v1.0.bin` in `data/models/tts/` (~300 MB total); truncated downloads are auto-rejected |
  | Audition run | 32 WAVs in `data/tts/audition/` (28-voice sweep + 4 persona samples); manifest printed personas-first |
  | Listen | Each of the 4 persona samples sounds distinct and fits the character; accept defaults or remap |
  | Sign-off | Record the final casting in this plan doc; `data/models/tts/` populated |

### M2: Real-engine smoke gate
- **Source step:** Step Z8 (from this plan's §7)
- **Issue:** #11
- **Commands to run:**
  ```powershell
  # backend with the tts extra installed (M1 prerequisite), 60-second end-to-end, NO stubs:
  uv run python -m toybox.db.migrate
  uv run python -m toybox.main --host 127.0.0.1 --port 8000
  # in the parent UI: propose + approve one real activity, then:
  #   dir data\tts   (worker-rendered real Kokoro WAVs appear under <voice>\)
  #   curl.exe -I http://127.0.0.1:8000/api/static/tts/<voice>/<sha16>.wav   (from the step's spoken_audio_url)
  ```
- **What you're looking for:**

  | Check | Expected outcome |
  |---|---|
  | Approve → render | Worker renders real WAVs into `data/tts/<voice>/` within ~5–10 s per step (INFO render counts in the log) |
  | Serve | `curl -I` on a `spoken_audio_url` returns 200 with `audio/wav` |
  | Voice identity | One clip spot-checked audibly the cast persona voice |
  | Stability | One full cycle completes without crashing (deliverable is the round-trip, not content quality) |

### M3: iPad UAT
- **Source step:** Step Z9 (from this plan's §7)
- **Issue:** #12
- **Commands to run:**
  ```powershell
  # launch for the iPad per the usual kiosk flow (scripts/launch-toybox.ps1 with LAN IP + parent PIN)
  ```
- **What you're looking for:**

  | Check | Expected outcome |
  |---|---|
  | (1) Two personas on Read Me | Audibly different voices |
  | (2) Long step body | Spoken audio matches on-screen text verbatim (Phase-R cutoff class gone) |
  | (3) Joke auto-play | Uses the persona clip voice; setup→punchline order preserved |
  | (4) `neural_voice_enabled` OFF | Device voice returns, with sentence-boundary truncation |
  | (5) Adventure beats | Speak (clip when rendered, Web Speech fallback otherwise — fallback is designed) |
  | (6) Gesture unlock from PIN gate | No dead first tap; ALSO verify a JokeStep AUTOPLAY clip plays without a preceding tap (cached-PIN reload skips priming — known risk) |
  | (7) Choice-label read-aloud | Plays clips (index-aligned per choice) |
  | Wrap | UAT run doc under `documentation/runs/`; defects filed as issues; non-blocking cosmetics may fold into the next bundle |
