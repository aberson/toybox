...
- **Depends on:** P6.
- **Status:** DEFERRED-TO-UAT as M1 (2026-05-19). Per operator choice in build-phase re-resume, P7 deferred to the phase-end Manual UAT bundle rather than halting mid-phase. P7b shipped with `IP_ADAPTER_SCALE = 0.6` (P4 default) + a value-pinning test; if operator's M1 smoke gate picks a different scale, a 2-line follow-up commit updates the constant + the pinning test in the same PR.
...
- **Depends on:** P7.
- **Status:** DONE (2026-05-19). `IP_ADAPTER_SCALE = 0.6` (P4 default, no run-doc value to apply because P7 deferred). New unit test `test_ip_adapter_scale_value_pinned` pins the literal value so a future change to the constant must update the test in the same PR. Per workspace `code-quality.md` § "One source of truth for data-shape constants".
...
- **Depends on:** P7b.
- **Status:** DEFERRED-TO-UAT as M2 (2026-05-19). Pure-observation operator step; deferred to the phase-end Manual UAT bundle per skill default (no downstream code consumer).
...
| P7 — Operator: smoke gate + UAT-tune IPA scale (run-doc only, no code edit) | DEFERRED-TO-UAT as M1 (2026-05-19) |
| P7b — Code: apply UAT-chosen IP_ADAPTER_SCALE to pipeline.py + pin test | DONE (2026-05-19) |
| P8 — Operator: iPad UAT (global regenerate + real activity) | DEFERRED-TO-UAT as M2 (2026-05-19) |
...
### M1: Smoke gate — 1 toy × 10 sprites + UAT-tune IPA scale value

- **Source step:** Step P7 (from this plan's §7)
- **Issue:** #189
- **Commands:**

  ```powershell
  # (a) Pick a live toy with a clear reference photo (Plush Unicorn from the
  # F.5 smoke history is a known-good candidate)
  $toyId = "<paste-toy-id-from-data/images/toys/>"

  # (b) Generate all 10 action sprites via the existing per-toy endpoint.
  # PIN-token needed for the parent endpoint; substitute $token below.
  $token = "<paste-parent-PIN-token>"
  Invoke-RestMethod -Method Post -Uri "http://127.0.0.1:8000/api/toys/$toyId/actions/regenerate" `
    -Headers @{ Authorization = "Bearer $token" }

  # (c) Inspect each sprite at data/images/toy_actions/<toy_id>/<slot>.png
  Get-ChildItem "data/images/toy_actions/$toyId/" | Select-Object Name, Length

  # (d) Optional VRAM + wall-clock capture during a single 10-slot run
  nvidia-smi --query-gpu=memory.used --format=csv
  ```

  Optional scale-tune procedure if 0.6 produces weak identity or collapsed pose:
  edit `IP_ADAPTER_SCALE` in `src/toybox/image_gen/pipeline.py` (try 0.4 to loosen, 0.8 to tighten),
  restart backend, regenerate, eyeball side-by-side. **Revert the edit** after each
  experiment so working tree is clean. Pick the best value and document it.

- **What to look for:**

  | Check | Expected outcome |
  |---|---|
  | (a) Sprite identity | Sprite visually resembles the toy in the reference photo |
  | (b) Pose-intent | Action is recognizable as the slot it represents (idle, cheering, etc.) |
  | (c) Quality floor | No obviously broken sprites (smeared, pseudo-text, wrong subject) |
  | (d) Wall-clock | < 5 s warm per sprite |
  | (e) VRAM | Peak < 7 GB on the 8 GB host |
  | (f) Floor | At least 7 of 10 slots PASS identity AND pose |

  If (f) passes at the default scale (0.6), commit a short run-doc at `documentation/runs/2026-MM-DD-phase-p-smoke.md` recording the per-sprite wall-clock + VRAM + the verdict. No code change.

  If a different scale value produces visibly better identity+pose, commit a 2-line follow-up to BOTH `src/toybox/image_gen/pipeline.py` (the `IP_ADAPTER_SCALE` constant) AND `tests/unit/image_gen/test_pipeline_stub.py` (the `test_ip_adapter_scale_value_pinned` expected literal). Reference the run-doc in the commit message.

  Close #189 when the run-doc lands.

### M2: iPad UAT — 2-3 toys × GLOBAL regenerate flow + kiosk render in real activity

- **Source step:** Step P8 (from this plan's §7)
- **Issue:** #191
- **Commands:**

  ```powershell
  # Backend + frontend should be running on the F.5-capable host. From the iPad:
  # 1. Open the parent UI at http://<TOYBOX_LAN_IP>:8000/ (or the configured LAN host)
  # 2. Ingest 2-3 fresh toys or use existing ones (Child A's LOL dolls; Child B's
  #    Periodic Table accessories per memory project_kids_profiles.md)
  # 3. Click the new GLOBAL "Regenerate sprites for every toy" button at the
  #    top of the parent toys-list page
  # 4. After completion, open the child kiosk + run a real activity that uses
  #    one of those toys' action sprites
  ```
...
- **What to look for:**
  ...
  | Check | Expected outcome |
  |---|---|
  | (a) GLOBAL button works | Confirmation dialog appears; response toast reports `total_enqueued` count |
  | (b) Per-slot WS badges | Each toy's grid shows queued → running → done transitions live |
  | (c) Per-toy identity | After completion, each toy's sprites visually represent that toy (better than pre-Phase-P state) |
  | (d) Kiosk render | Real activity renders the new 512-source sprite at the expected display size WITHOUT pixelated chunkiness |
  | (e) Kid recognition | Both kids recognize their toys in at least one rendered slot (Child A: LOL doll cheering; Child B: toy looking) |
  | (f) No crashes | Backend log shows no `c10.dll`, `OutOfMemoryError`, or capability-gate flips |
  | (g) Floor | Identity + pose pass on ≥ 70% of generated slots |

  Document at `documentation/runs/2026-MM-DD-phase-p-uat.md` with operator judgment + kid-feedback + 2-3 iPad screenshots. Cosmetic per-toy quality misses are NOT blockers — file follow-up GitHub issues for those and close #191.

  This closes Phase P. Run `/repo-update` to push the run-docs + close umbrella #182 afterward.
