...
*Generated by /build-phase on 2026-06-22. Append-only; re-running the phase adds new items below, never modifies existing ones.*

### M1: Render the scene library (operator)
- **Source step:** Step Y2-run (from this plan's §7)
- **Issue:** #267
- **Commands to run:**
  ```powershell
  # Stop the backend FIRST — the scene batch and the live sprite pipeline both
  # load CUDA and will OOM if both run at once (Phase U U3 lesson).
  uv run python scripts/batch_scenes.py
  # (optional) preview without rendering:
  uv run python scripts/batch_scenes.py --dry-run
  ```
- **What you're looking for:**

  | Check | Expected outcome |
  |---|---|
  | Scene PNGs written | one `data/images/scenes/<id>.png` per `SCENE_IDS` member (~8–10) |
  | Age-appropriateness | every scene is child-safe / age-appropriate (one-time parent eyeball) |
  | Style cohesion | scenes read as the SAME cartoon style as the toy sprites (not a "ransom-note" mismatch) |

### M2: iPad UAT (operator)
- **Source step:** Step Y9 (from this plan's §7)
- **Issue:** #274
- **Commands to run:**
  ```powershell
  # Bring up the app (loopback) + frontend, then open the kiosk on the iPad.
  uv run python -m toybox.main --host 127.0.0.1 --port 8000
  # in another shell:
  cd frontend; npm run dev
  # Propose + approve an activity for Child A, then for Child B, and open the child kiosk.
  ```
- **What you're looking for:**

  | Check | Expected outcome |
  |---|---|
  | Backdrop renders | a full-bleed scene image sits BEHIND the step card |
  | Readability | body text stays readable over the scene (scrim + card opacity hold) |
  | Cast in-style | the toy sprites look like they're IN the scene, not pasted on |
  | Per-child differs | an interest-selected scene differs between Child A (dance/stage) and Child B (lab/space) |
