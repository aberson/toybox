...
| **E** — local model + tool-loop (post-v1) | tool-loop refactor + locally-hosted SFT (Supervised Fine-Tuned) model swap-in for Claude OAuth | IN FLIGHT — two backend substrate carve-outs shipped ahead of the gated-on-data full ship. Step 28 carve-out 2026-05-05 (commit `33a4b3c`: tool registry + ClaudeActivityGenerator wrapper + env-var dispatch + `is_local_capable()` stub). Step 27 (E3) backend carve-out 2026-05-13 (commit `4f735a0`: PII redactor `src/toybox/ai/redact.py` + migration 0013 `redact_for_sft` opt-out column + `eval_dump.py --sft-export` mode + `data/models/lora/REGISTRY.md` template + end-to-end smoke gate; see [`e3-backend-carveout-plan.md`](e3-backend-carveout-plan.md)). Full-ship remainder gated on ≥50 SFT-filter rows; resume Step 27 when threshold crossed |
