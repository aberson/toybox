...
| [phase-m-plan.md](phase-m-plan.md) — Periodic Table Professor + SEL (content depth) | ✅ COMPLETE 2026-05-18 at master `a096e11` — autonomous block + M2b sprite render + M7b TTS render + M14 iPad UAT all shipped. UAT verdict: 11/12 PASS, 1 DEFERRED (row #4 `shrink_into_helium_balloon_voyage` blocked by D2, retest scheduled in Phase N N6 #176). Umbrella [#152](https://github.com/aberson/toybox/issues/152) closed; 3 defects + 1 feature request folded into Phase N + Phase O. Run-doc: [`runs/2026-05-18-phase-m-uat.md`](runs/2026-05-18-phase-m-uat.md). |
| [phase-n-plan.md](phase-n-plan.md) — Element microgame template shape (4-step + 2-fork) + Phase M defect fold-ins | 📋 STAGED 2026-05-18 — 9 steps (N0, N0b, N1-prep, N1, N2-N6). Umbrella [#167](https://github.com/aberson/toybox/issues/167) + step issues [#168-#176](https://github.com/aberson/toybox/issues/167) minted. Plan-review + plan-wrap complete. **Blocker gate: N0 (#168) — unblocks Phase M UAT row #4 retest** at N6 (#176). Parallel-safe kickoff: N0, N0b, N1-prep, N3 can start in parallel worktrees. |
...
Build order: Phase A → B → C → D → iPad-Kiosk → F.5 → G → H → I → J → K → L → M ✅ (all shipped). Phase N (element_microgame template shape + Phase M defect fold-ins D1+D2) and Phase O (parent UX 5-tab refresh) staged 2026-05-18 — 14 GH issues minted (#167-#180), plan docs at [phase-n-plan.md](phase-n-plan.md) + [phase-o-plan.md](phase-o-plan.md). Phase E (local model + tool-loop) is in flight — Step 28 + Step 27 (E3) carve-outs shipped 2026-05-05 / 2026-05-13; full-ship remainder gated on ≥50 SFT-filter rows in `labeled_events`.
