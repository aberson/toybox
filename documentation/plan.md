...
- **Issue:** #27
- **Flags:** --reviewers full --ui
- **Status:** DONE (2026-05-03, commit `__PENDING__`) — backend DELETE endpoints + frontend manager + 17 backend tests + 10 frontend tests all green. Notable: extracted `enforce_pin_check(pin, conn, rate_limiter)` from `auth.py::post_parent` so the wipe-all endpoint shares the same lock-precedence + verify + counter logic without duplication; the helper does NOT call `record_successful_attempt` (login still does). Wipe-all uses `DELETE` with a JSON body — unconventional but FastAPI accepts it and the alternative (PIN-in-header) is harder to type-check on the frontend. Optimistic delete restores the row on non-404 errors; 404 keeps the row removed and surfaces a subdued "already deleted" notice rather than treating it as a hard failure. Cursor pagination only enables "Load more" for the unfiltered list (search returns up to `limit` matches without a cursor in v1).
