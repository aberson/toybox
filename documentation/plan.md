...
**v1 shipped 2026-05-02** at the end of Phase A — closed-loop demo with a manual "trigger" button instead of a real mic; adult-only testing. The system has since shipped phases B (hearing), C (content), D (polish), iPad-Kiosk (PWA on iPad), F.5 (image-gen sprite pipeline), G (branching gameplay), H (parent UX revamp), I (transcript retention + display refresh), and J (autonomous play queue) and is in family-private testing on real children. **Concurrent post-v1 work:** Phase E (local model + tool-loop, IN FLIGHT — backend substrate carve-outs done) and Phase K (roles + songs + jokes + voice, IN FLIGHT — K1-K15 substrate shipped 2026-05-15; K16 backfill soak + K17 smoke gate + K18 iPad UAT remaining).
...
| **K** — roles + songs + jokes + voice | toy role taxonomy (10 roles) + slot-fill engine + proposed-only recast; pre-rendered song corpus (Coqui TTS) + Web Speech joke corpus across 5 delivery surfaces (standalone / theme-tagged embedded / endings / parent-inserted / persona-or-character spontaneity); click-to-read on kiosk (word taps + watermarked Read Me button); 8 parent feature flags; 200 existing templates backfilled via overnight 4-agent soak | IN FLIGHT — K1-K15 substrate shipped 2026-05-15 (issues [#114-#128](https://github.com/aberson/toybox/issues/114) closed; +132 backend pytest, +123 frontend vitest); K16 (overnight 4-agent backfill of 200 templates), K17 (end-to-end smoke gate; requires `pip install TTS && python scripts/generate_song_corpus.py` to render `data/songs/audio/*.mp3` first), K18 (M1 iPad operator UAT) deferred to a fresh session. Umbrella [#113](https://github.com/aberson/toybox/issues/113); full design in [`phase-k-plan.md`](phase-k-plan.md). **Phase K6 + K15 touched [activities.py](../src/toybox/api/activities.py); Phase E sequencing already accounted for in the row above.** |
