...
| [phase-m-plan.md](phase-m-plan.md) — Periodic Table Professor + SEL (content depth) | 🟡 AUTONOMOUS + M2b COMPLETE 2026-05-18 at master `ad5c5f7` — M1, M2a, M3-M13 + M2b shipped (mid-session prompt+overlay redesign: SD 1.5 can't render legible glyphs, swapped to Pillow text overlay over orb-of-element diffusion; 14 canonical sprites in git, 104 local). M7b Coqui TTS audio render + M14 iPad UAT staged for operator session (run doc: [`runs/2026-05-18-phase-m-uat.md`](runs/2026-05-18-phase-m-uat.md), 12 curated activities). Umbrella [#152](https://github.com/aberson/toybox/issues/152) stays open until M7b + M14 close. |
...
Build order: Phase A → B → C → D → iPad-Kiosk → F.5 → G → H → I → J → K → L ✅ (all shipped). Phase M (content depth — Periodic Table + SEL) autonomous block + M2b shipped 2026-05-18 at `ad5c5f7`; M7b + M14 pending the bundled operator session. Phase E (local model + tool-loop) is in flight — Step 28 + Step 27 (E3) carve-outs shipped 2026-05-05 / 2026-05-13; full-ship remainder gated on ≥50 SFT-filter rows in `labeled_events`.
